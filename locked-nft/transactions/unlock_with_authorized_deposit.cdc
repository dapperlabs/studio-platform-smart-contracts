...
/// This transaction adds NFTs to an escrow leaderboard, unlocking them if necessary.
///
transaction(leaderboardName: String, nftIDs: [UInt64]) {
    ...
    prepare(owner: auth(Storage, Capabilities) &Account) {
        ...
        self.collectionPublic = escrowAccount.capabilities.borrow<&Escrow.Collection>(Escrow.CollectionPublicPath)
    ...
    execute {
        ...
        // Add each NFT to the leaderboard
        for nftID in nftIDs {
            ...
            if self.userLockerCollection != nil && NFTLocker.getNFTLockerDetails(id: nftID, nftType: nftType) != nil {
                ...
                if NFTLocker.canUnlockToken(id: nftID, nftType: nftType) {
                    self.collectionPublic.addEntryToLeaderboard(
                        nft: userLockerCollection!.unlock(id: nftID, nftType: nftType),
                        leaderboardName: leaderboardName,
                        ownerAddress: self.ownerAddress,
                    )
                } else {
                    userLockerCollection!.unlockWithAuthorizedDeposit(
                        id: nftID,
                        nftType: nftType,
